...
# trace algorithm, see traceCode.py
...
def onAppStart(app):
    ...
    app.green, app.red, app.textColor, app.lightPath, 
    app.darkPath) = loadColors(app.colorSchemeIsLight)
    
    app.background = app.primary 
    app.colorSchemeSwitcherPath = app.lightPath if app.colorSchemeIsLight else app.darkPath
...
def main_onMousePress(app, mouseX, mouseY):
    ...
    im_width, im_height = getScaledImageSize(app.colorSchemeSwitcherPath, 7)
...
def main_redrawAll(app):
    ...
    # starter message
    if app.grid == None:
        drawLabel(f'Select a file from the file explorer to get started!', 
...
def drawColorschemeSwitcher(app, x, y):
    ...
    drawImage(app.colorSchemeSwitcherPath, x, y, 
...
def fileTree_onKeyPress(app, key):
    ...
    if key == 'up' and app.selectedFileIndex > 0:
        ...
        fullPath = getCurrentFilePath(app)
        if os.path.isdir(fullPath):
            os.chdir(fullPath)
            app.fileStack.append(fullPath)
            app.files = listFiles()
